<script>
	...
	function handleTableFinalize(e, i, j) {
        ...
        if (items.length > 1) { // dropped
            ...
            if (originalCellContent.name.trim() !== '') { // swap
                if (info.source.id === 'draggable-list') {
                    isSwapping = true;
                    draggableItems = [...draggableItems.filter(item => item.id !== droppedItem.id), originalCellContent];
        ...
        } else if (items.length === 0) { // dragged out
...
<div class="card">
    ...
    >
        {#each draggableItems as item (item.id)}
            <div id="draggable-item-{item.id}" animate:flip={{ duration: flipDurationMs }}>{item.name}</div>
        {/each}
    </section>
</div>
