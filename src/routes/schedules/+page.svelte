...
<div class="flex flex-col">
	...
	<section class="relative bg-muted/30 py-16 md:py-24 overflow-hidden border-b">
		...
		<div
			class="container relative z-10 mx-auto px-4 flex flex-col items-center text-center space-y-6"
		>
			...
			<p class="text-lg md:text-xl text-muted-foreground max-w-2xl">
				Browse the latest published timetables for all colleges and programs. Search for your
				specific block, instructor, or room to see where you need to be.
			</p>
			...
			<div class="w-full max-w-2xl mt-8 relative group">
				...
				></div>
				<div
					class="relative flex items-center bg-card border shadow-lg rounded-full overflow-hidden p-1"
				>
	...
	<section class="container mx-auto px-4 py-12">
		<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
			<div>
				...
				<p class="text-muted-foreground text-sm mt-1">
					Select a timetable to view its full schedule grid.
				</p>
			</div>
		...
		{#if filteredTimetables.length === 0}
			<div
				class="flex flex-col items-center justify-center p-12 border rounded-xl bg-muted/10 border-dashed"
			>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each filteredTimetables as target}
				...
				<a href={`/schedules/${target.id}`} class="block h-full cursor-pointer">
					<Card.Root
						class="h-full group hover:border-primary/50 hover:shadow-md transition-all relative overflow-hidden {isArchived
							? 'grayscale-[0.3] hover:grayscale-0'
							: ''}"
					>
						{#if !isArchived}
							<div
								class="absolute top-0 right-0 w-24 h-24 bg-primary/10 rounded-bl-full -mr-8 -mt-8 transition-transform group-hover:scale-110"
							></div>
						{/if}
						<Card.Header>
							<div class="flex justify-between items-start mb-2">
								<Badge
									...
									class={!isArchived ? 'bg-primary text-primary-foreground shadow-sm' : ''}
								>
						...
						<Card.Content>
							<div class="grid grid-cols-2 gap-4 text-sm text-muted-foreground">
								<div class="flex items-center">
									<BookOpen
										class="w-4 h-4 mr-2 {!isArchived ? 'text-primary' : 'text-muted-foreground'}"
									/>
						...
						<Card.Footer class="pt-2">
							<Button
								...
								class="w-full {isArchived ? 'border shadow-sm' : ''} {isArchived
