...
const handleSearch = async <T>(promise: Promise<{ data: T[] | null; error: PostgrestError | null }>): Promise<T[]> => {
...
export const GET: RequestHandler = async ({ url, locals }) => {
    ...
    if (profile.role === 'Dean' || profile.role === 'Chairperson') {
        if (profile.college_id) {
            ...
            roomsQuery = roomsQuery.or(`owner_college_id.eq.${profile.college_id},is_general_use.eq.true`);
        }
    }
